...
from scripts.config import IMAGE_DIR, CHECKPOINT_DIR, PRED_DIR, TEST_LIST_FILE, CROP_DIR
from utils.model import get_segmentor
from utils.dataset import PetDataset

model_name = 'unet'
model_path = os.path.join(CHECKPOINT_DIR, f'{model_name}_seg_cam_0.5_epoch_15.pth')
...
with torch.no_grad():
    for idx in range(int(len(dataset)*0.1)):
        image = dataset[idx]
        image_tensor = image.unsqueeze(0).to(device)

        output = model(image_tensor)  

        pred_mask = output.squeeze().cpu().numpy()  # shape: [H, W]
